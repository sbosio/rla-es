...
if [ "$PUBLICAR" == "SÍ" ] ; then
  ...
  echo -ne "\nVamos a crear en el repositorio git la etiqueta $vCORRECTOR, ¿está seguro? (s/n): "
  read -r  -n 1 RESPUESTA
  if [ "$RESPUESTA" = "s" ] || [ "$RESPUESTA" = "S" ]; then
  ...
  echo -ne "\nHay que subir al repositorio origen todos los cambios de la versión $vCORRECTOR, ¿está seguro? (s/n): "
  read -r  -n 1 RESPUESTA
  if [ "$RESPUESTA" = "s" ] || [ "$RESPUESTA" = "S" ]; then
    echo -e "\nejecutando: git tag -a $vCORRECTOR"
    git tag -a "$vCORRECTOR" || exit 1
